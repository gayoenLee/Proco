...
class AppDelegate: UIResponder, UIApplicationDelegate,  UNUserNotificationCenterDelegate{
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        if is_foreground_msg == "ok"{
                ...
                case "friend_request":
                print("친구 요청 관련 노티")
                    ViewRouter.get_view_router().current_page = .friend_volleh
                    ViewRouter.get_view_router().fcm_destination = "manage_friend"
                    break
                //6.친구가 좋아요 표시한 경우
                case "friend_card_like":
                print("친구가 좋아요 표시한 경우")
                    ViewRouter.get_view_router().current_page = .notice_tab
                    ViewRouter.get_view_router().fcm_destination = "notice_tab"
                    break
                //7.내가 참가한 친구 카드 수정시
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, willPresent notification: UNNotification, withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Void) {
        ...
        print("포그라운드 푸시일 때: \(notification.request.content.userInfo)")
        ...
        if is_foreground_msg == "ok"{
            ...
            case "chat":
                print("채팅 메세지인 경우")
                let chatroom_idx_string = userInfo["chatroom_idx"] as? String
                let chatroom_idx = Int(chatroom_idx_string!)
                print("채팅방: \(chatroom_idx)")
