...
struct FeedDisclosureSettingView: View {
    ...
    var body: some View {
        ...
        .onDisappear{
            ...
            if self.selected == "전체 공개" && self.main_vm.user_info_model.calendar_public_state != 0{
            ...
            }else if self.selected == "카드만 공개" &&  self.main_vm.user_info_model.calendar_public_state != 1{
                print("이전 값과 달라서 편집 통신")

                self.main_vm.user_info_model.calendar_public_state = 1
                self.main_vm.edit_calendar_disclosure_setting(calendar_public_state: self.main_vm.user_info_model.calendar_public_state)
                
            }else if self.selected == "비공개" &&   self.main_vm.user_info_model.calendar_public_state != 2{
                print("이전 값과 달라서 편집 통신")
