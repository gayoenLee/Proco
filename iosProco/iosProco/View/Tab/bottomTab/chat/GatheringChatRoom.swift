...
struct GatheringChatRoom: View {
    ...
    //채팅방 설정 -> 방 이름 변경시 전달하기 위함.(사용 모델이 달라서 파라미터로 전달하는 것으로 함.)
    ...
    var body: some View {
        ZStack{
            VStack{
                ...
                NavigationLink("",
                               destination: GroupVollehMainView(),
            ...
            //서버에 unread이벤트 전달
            .onAppear{
                print("현재 모임 enter chatroom idx 채팅방 idx: \(socket.enter_chatroom_idx)")
                let my_idx = Int(ChatDataManager.shared.my_idx!)
                print("모여볼래 서버에 보내기 전에 idx확인: \(String(describing: ChatDataManager.shared.my_idx))")
                print("모여볼래 채팅방 입장시 user read에 보내는 read last idx: \(ChatDataManager.shared.read_last_message)")
                ...
                socket.enter_friend_card_chat(server_idx:  server_idx, user_idx: my_idx!,chatroom_idx: SockMgr.socket_manager.enter_chatroom_idx, nickname:ChatDataManager.shared.my_nickname!, profile_photo_path: "", read_start_idx: ChatDataManager.shared.read_start_message, read_last_idx: ChatDataManager.shared.read_last_message, updated_at: updated_at, deleted_at: "")
                
                //내가 마지막으로 읽은 메세지 최신 메세지idx로 업데이트(나)
                ChatDataManager.shared.update_user_read( chatroom_idx: socket.enter_chatroom_idx, read_last_idx: ChatDataManager.shared.last_message_idx, user_idx: my_idx!, updated_at: updated_at)
                
                //새로운 채팅 메세지가 왔을 때 어떤 뷰에 있느냐에 따라 노티피케이션을 띄워주는게 다르기 때문에 알기 위해 사용.
                //채팅 목록 페이지 : 222, 채팅방 안: 333(기본: 111)
                socket.current_view = 333
                
                //메시지별로 안읽은 갯수 표현하기 위해 계산하는 것.
                if  ChatDataManager.shared.get_read_last_list(chatroom_idx: socket.enter_chatroom_idx){
                    print("모여 볼래에서 채팅방 마지막 메세지 idx 가져옴: \(ChatDataManager.shared.user_read_list)")
                }
                
                //채팅 메세지 데이터 가져오기
                ChatDataManager.shared.get_message_data(chatroom_idx: socket.enter_chatroom_idx, user_idx: Int(ChatDataManager.shared.my_idx!)!)
                //안읽은 메세지 계산 메소드.
                ChatDataManager.shared.calculate_last()
                
                //채팅 방 안 유저 정보들 가져와서 데이터 모델에 넣기(드로어에 보여줌)
                ChatDataManager.shared.read_chat_user(chatroom_idx: socket.enter_chatroom_idx)
                print("현재 채팅방 idx: \(socket.enter_chatroom_idx)")
                
                //현재 채팅방의 주인 idx를 이용해 닉네임 가져와서 상단에 표시하기, 드로어 예외처리에 사용.
                ChatDataManager.shared.get_creator_nickname(chatroom_idx: socket.enter_chatroom_idx)
                
                //드로어에서 카드 정보 보여주기 예외 처리 위해서 채팅방 정보 가져오기
                //                ChatDataManager.shared.read_chatroom(chatroom_idx: socket.enter_chatroom_idx)
                
            }
            .onDisappear(perform: {
                print("모여볼래 채팅룸 사라짐")
                socket.exit_chatroom(chatroom_idx: socket.enter_chatroom_idx)
                socket.chat_message_struct.removeAll()
