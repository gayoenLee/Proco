    ...
    var body: some View{
        VStack{
                HStack{
                    ...
                    NavigationLink("", destination: GroupVollehMainView(main_vm: self.main_vm)   .navigationBarHidden(true)
                                    .navigationBarTitle(""), isActive: self.$go_to_edit)
                    Button(action: {
        ...
        ScrollView{
            VStack{
                EditingView(main_vm: self.main_vm, category_alert: self.$category_alert, is_title_empty: self.$is_title_empty, is_offline_meeting: self.$is_offline_meeting, selected_category: self.$selected_category, open_map: self.$open_map, selected_img: self.$selected_image, show_img_picker: self.$show_img_picker, go_to_edit: self.$go_to_edit)
            }
        }
        //갤러리 나타나는 것.
