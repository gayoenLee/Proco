...
struct FilterCategoryView: View {
    ...
    @Binding var tag_num_over_three : Bool
    //카테고리는 유저가 직접 작성한 태그와 구분해서 보내고 저장해야함.
    @Binding var selected_category : Set<String>
            ...
            Capsule()
                ...
                .overlay(
                    ...
                    Button(action: {
                        ...
                        self.tag_num_over_three = viewmodel.limit_filter_category_num(tag_list: self.viewmodel.selected_filter_tag_list)
                        ...
                        if tag_num_over_three{
                            ...
                            self.selected_category.insert(category_model.category_name!)
                            print("태그 카테고리 뷰모델에 넣어졌는지 확인 : \(self.viewmodel.selected_filter_tag_list)")
                        }
                    }){
                        HStack{
                            Image(self.viewmodel.selected_filter_tag_set.contains(category_model.category_name!) ? "category_checked" : "tag_plus")
                                .resizable()
